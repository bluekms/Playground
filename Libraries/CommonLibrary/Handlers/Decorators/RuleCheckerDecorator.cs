...
public class RuleCheckerDecorator<TRule> : IRuleChecker<TRule>
    where TRule : IRule
{
    private static readonly TimeSpan Threshold = TimeSpan.FromMilliseconds(100);

    private readonly ActivitySource activitySource;
    private readonly IRuleChecker<TRule> checker;
    private readonly ILogger<RuleCheckerDecorator<TRule>> logger;
    private readonly IHostEnvironment hostEnvironment;

    public RuleCheckerDecorator(
        ActivitySource activitySource,
        IRuleChecker<TRule> checker,
        ILogger<RuleCheckerDecorator<TRule>> logger,
    ...
    public async Task CheckAsync(TRule rule, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await checker.CheckAsync(rule, CancellationToken.None);
            sw.Stop();

            if (sw.Elapsed < Threshold)
            {
                LogInformation(logger, rule, sw.Elapsed.TotalMilliseconds, null);
            }
            else
            {
                ...
                {
                    LogWarningForDev(logger, rule, sw.Elapsed.TotalMilliseconds, null);
                }
            }
        }
        catch (Exception e)
    ...
    private static readonly Action<ILogger, TRule, double, Exception?> LogInformation =
        LoggerMessage.Define<TRule, double>(
            LogLevel.Information,
            EventIdFactory.Create(ReservedLogEventId.RuleCheckerInformation),
            "Rule: {@Rule}, Elapsed: {Elapsed:0.0000}ms");
    ...
    private static readonly Action<ILogger, TRule, double, Exception?> LogWarningForDev =
        LoggerMessage.Define<TRule, double>(
            LogLevel.Warning,
            EventIdFactory.Create(ReservedLogEventId.RuleCheckerWarning),
            "Rule Too Slow. Rule: {@Rule}, Elapsed: {Elapsed:0.0000}ms");
