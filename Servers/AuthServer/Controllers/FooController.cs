    ...
    public class FooController : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> HandleAsync([FromBody] ArgumentData args)
        {
            return args.Command switch
            {
                ...
                "Test1" => await Utf8JsonTest1(),
                "Test2" => await Utf8JsonTest2(),
                "Test3" => await Utf8JsonTest3(),
                "Test4" => await Utf8JsonTest4(args.Data),
                _ => throw new InvalidOperationException(),
            };
        }
        
        private async Task<ActionResult<string>> Utf8JsonTest1()
        {
            var data = new LocaleLanguage(100, new List<string>() { "aaa", "bb", "cccccffff" });
            var s = Utf8Json.JsonSerializer.Serialize(data);
            var d = Utf8Json.JsonSerializer.Deserialize<LocaleLanguage>(s);
            
            var ss = JsonSerializer.Serialize(data);
            var sd = JsonSerializer.Deserialize<LocaleLanguage>(ss);

            return "Ok";
        }
        
        private async Task<ActionResult<string>> Utf8JsonTest2()
        {
            var data = new LocaleLanguage(1, new()
            {
                @"multilingual test",
                @"다국어 테스트",
                @"多言語テスト",
        ...
        private async Task<ActionResult<string>> Utf8JsonTest3()
        {
            var data = new LocaleLanguage2(2, 99, 88, 77, 66, 55, 44, 33);

            var s = Utf8Json.JsonSerializer.Serialize(data);
            var d = Utf8Json.JsonSerializer.Deserialize<LocaleLanguage2>(s);
            
            var ss = JsonSerializer.Serialize(data);
            var sd = JsonSerializer.Deserialize<LocaleLanguage2>(ss);

            return "Ok";
        }

        private async Task<ActionResult<string>> Utf8JsonTest4(string value)
        {
            var data = new LocaleLanguage(99, new()
            {
                @"multilingual test",
                @"다국어 테스트",
                @"多言語テスト",
        ...
        public sealed record LocaleLanguage(int Id, List<string> Text);
        
        public sealed record LocaleLanguage2(int Id, int 값, int Value, int ценность, int মান, int 价值, int 價值, int ค่า);

        public sealed record ArgumentData(string Command, int Id, string Data);
    }
}